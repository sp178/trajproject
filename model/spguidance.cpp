...
int spguidance::onInitial()
{
	...
	string  clpath, cdpath, czpath, aerodempath,speedpath,
		clname, cdname, czname, aerodemname,speedname;
	ptree _table;
	ptree _null;
	...
	m_interCL = makeBlock<2>(clpath, clname);
	m_interCD = makeBlock<2>(cdpath, cdname);
	m_interCZ = makeBlock<2>(czpath, cdname);
	m_interpho= makeBlock<1>(aerodempath, aerodemname);
	m_interma = makeBlock<1>(speedpath, speedname);
